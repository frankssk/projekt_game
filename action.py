...
def howPot():
    print(f'Маленьких: {ac.playerItem["potMin"]} Средних: {ac.playerItem["potStand"]} Больших: {ac.playerItem["potMax"]} Свитки воскрешения(пока не работает): {ac.playerItem["scroll"]}')
    command = str(input('Какое зелье вы хотите использовать?\n Маленькое 10% = 1\n Среднее 50% = 2\n Большое 100% = 3\n Отменить выбор = 4\n'))
    while True:
        if command == '1':
